...
namespace UnrealMono
{
	...
	public static class DrawGroup
	{
	    ...
	    public const float Enemies1 = 0.4F;
	    public const float Enemies2 = 0.45F;
	    public const float Enemies3 = 0.5F;
        public const float NPCs = 0.6F;
		public const float Shots = 0.65F;
		public const float Players = 0.7F;
		public const float ForegroundParticles = 0.75F;
		public const float Foreground = 0.8F;
#if DEBUG
		public const float DebugGraphics = 0.85F;
#endif
	...
	public class CSceneComponent : CComponent
	{
        ...
        /// Get the mass of this and every descendant component, scaled by their scale
        //TO Should be called weight
				...
				foreach (CSceneComponent foundSceneComponent in foundSceneComponents)
				{
						...
						&& foundSceneComponent.overlappingGroups.HasFlag((ComponentCollisionGroup)collisionGroup)) //TO1 check if root components is the same or owner?
					{
						if (CollisionCircle.IsCollidingWith(foundSceneComponent.CollisionCircle))
						{
							...
							{
#if DEBUG
								if (UnrealMonoGame.debugType.HasFlag(DebugType.OverlapCollisions))
								{
									...
									UnrealMonoGame.AddDebugString(first + " >-< " + second, false, 1F);
								}
#endif
