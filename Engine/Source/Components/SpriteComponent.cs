...
namespace UnrealMono
{
    public class CSpriteComponent : CSceneComponent
    {
			...
			: base(owner, parentSceneComponent, updateGroup, layerDepth, location, isLocationWorld, relativeScale, acceleration, maxSpeed)
		{
			...
			SetFramesAndTexture(frames, texture);
		}
		...
		public void SetFrames(List<Rectangle> frames)
		{
			...
			for (int k = 0; k < frames.Count; k++)
			{
				...
				float frameRadius = (float)Math.Sqrt(frameRadiusSquared);
				framesCollisionRadius.Add(frameRadius);
				frameRadiusAverage += frameRadius;
			}
			frameRadiusAverage /= frames.Count;
			Mass = frameRadiusAverage / 16F;
		}
		public void SetFramesAndTexture(List<Rectangle> frames, Texture2D texture)
		{
			SetTexture(texture);
			SetFrames(frames);
		}
		...
		protected override void Draw()
		{
			if (IsInViewport)
			{
				UnrealMonoGame.spriteBatch.Draw(
					texture,
					ScreenCenter,
...
#if DEBUG
			if (UnrealMonoGame.debugType.HasFlag(DebugType.Collisions)) //Show Wireframe Collision Boxes (Also select wireframe color???)
			{
				UnrealMonoGame.spriteBatch.DrawWorldCircle(WorldLocation, CollisionRadius * 2, 16, Color.White, 1, DrawGroup.DebugGraphics);

				UnrealMonoGame.spriteBatch.DrawWorldLine(new Vector2(BoundingBox.Left, BoundingBox.Top), new Vector2(BoundingBox.Right, BoundingBox.Top), Color.White, 1, DrawGroup.DebugGraphics);
				UnrealMonoGame.spriteBatch.DrawWorldLine(new Vector2(BoundingBox.Left, BoundingBox.Bottom), new Vector2(BoundingBox.Right, BoundingBox.Bottom), Color.White, 1, DrawGroup.DebugGraphics);
				UnrealMonoGame.spriteBatch.DrawWorldLine(new Vector2(BoundingBox.Left, BoundingBox.Top), new Vector2(BoundingBox.Left, BoundingBox.Bottom), Color.White, 1, DrawGroup.DebugGraphics);
				UnrealMonoGame.spriteBatch.DrawWorldLine(new Vector2(BoundingBox.Right, BoundingBox.Top), new Vector2(BoundingBox.Right, BoundingBox.Bottom), Color.White, 1, DrawGroup.DebugGraphics);
			}
#endif
		}
	}
