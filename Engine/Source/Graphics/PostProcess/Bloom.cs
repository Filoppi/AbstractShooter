...
namespace BloomPostprocess
{
	public class Bloom
	{
		...
		public Bloom()
		{
			format = UnrealMonoGame.Get.GraphicsDevice.PresentationParameters.BackBufferFormat;
		}

		public void LoadContent()
		{
			bloomExtractEffect = UnrealMonoGame.Get.Content.Load<Effect>(@"Shader\BloomExtract");
			bloomCombineEffect = UnrealMonoGame.Get.Content.Load<Effect>(@"Shader\BloomCombine");
			gaussianBlurEffect = UnrealMonoGame.Get.Content.Load<Effect>(@"Shader\GaussianBlur");
		}
		...
		public void ChangeResolution(int X, int Y)
		{
			...
			renderTarget1 = new RenderTarget2D(UnrealMonoGame.Get.GraphicsDevice, width, height, false, format, DepthFormat.None);
			renderTarget2 = new RenderTarget2D(UnrealMonoGame.Get.GraphicsDevice, width, height, false, format, DepthFormat.None);
		}
		...
		public void Draw(RenderTarget2D sourceRenderTarget, RenderTarget2D destRenderTarget)
		{
			...
			UnrealMonoGame.Get.GraphicsDevice.SetRenderTarget(destRenderTarget);
		...
		{
			UnrealMonoGame.Get.GraphicsDevice.SetRenderTarget(renderTarget);
		...
		private void DrawFullscreenQuad(Texture2D texture, int width, int height, Effect effect, IntermediateBuffer currentBuffer)
		{
			...
			UnrealMonoGame.Get.GraphicsDevice.Clear(Color.TransparentBlack); //<-- MUST do this for each target if using transparent target.
			UnrealMonoGame.spriteBatch.Begin(0, BlendState.AlphaBlend, null, null, null, effect);
			UnrealMonoGame.spriteBatch.Draw(texture, new Rectangle(0, 0, width, height), Color.White);
