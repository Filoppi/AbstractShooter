...
namespace AbstractShooter.States
{
	public abstract class MenuState : GUIState
	{
		...
		public override void Update(GameTime gameTime)
		{
			...
			{
				SoundsManager.PlaySoundEffect("Selection1"); //TO1
			}
		}
...
#if DEBUG
			/*int cc = 39;
            Point currentResolution = UnrealMonoGame.currentResolution;
            currentResolution.X = (int)Math.Round((double)(currentResolution.Y * UnrealMonoGame.expectedAspectRatio));
            //BottomRight
            UnrealMonoGame.spriteBatch.DrawLine(currentResolution.X - cc, currentResolution.Y, currentResolution.X, currentResolution.Y, Color.Yellow, 1F);
            UnrealMonoGame.spriteBatch.DrawLine(currentResolution.X, currentResolution.Y - cc, currentResolution.X, currentResolution.Y, Color.Yellow, 1F);

            //BottomLeft
            UnrealMonoGame.spriteBatch.DrawLine(0, currentResolution.Y, cc, currentResolution.Y, Color.Yellow, 1F);
            UnrealMonoGame.spriteBatch.DrawLine(1, currentResolution.Y - cc, 1, currentResolution.Y, Color.Yellow, 1F);

            //TopLeft
            UnrealMonoGame.spriteBatch.DrawLine(0, 1, cc, 1, Color.Yellow, 1F);
            UnrealMonoGame.spriteBatch.DrawLine(1, 0, 1, cc, Color.Yellow, 1F);

            //TopRight
            UnrealMonoGame.spriteBatch.DrawLine(currentResolution.X - cc, 1, currentResolution.X, 1, Color.Yellow, 1F);
            UnrealMonoGame.spriteBatch.DrawLine(currentResolution.X, 0, currentResolution.X, cc, Color.Yellow, 1F);*/
#endif
			base.EndDraw();
		}
