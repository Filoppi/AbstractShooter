...
namespace BloomPostprocess
{
    public class Bloom
    {
        ...
        public Bloom()
        {
            format = Game1.Get.GraphicsDevice.PresentationParameters.BackBufferFormat;
        }
        
        public void LoadContent()
        {                        
            bloomExtractEffect = Game1.Get.Content.Load<Effect>(@"Shaders\BloomExtract");
            bloomCombineEffect = Game1.Get.Content.Load<Effect>(@"Shaders\BloomCombine");
            gaussianBlurEffect = Game1.Get.Content.Load<Effect>(@"Shaders\GaussianBlur");
        }        
        ...
        public void ChangeResolution(int X, int Y)
        {
            ...
            renderTarget1 = new RenderTarget2D(Game1.Get.GraphicsDevice, width, height, false, format, DepthFormat.None);
            renderTarget2 = new RenderTarget2D(Game1.Get.GraphicsDevice, width, height, false, format, DepthFormat.None);
        }
        ...
        {        
            ...
            Game1.Get.GraphicsDevice.SetRenderTarget(destRenderTarget);
        ...
        {
            Game1.Get.GraphicsDevice.SetRenderTarget(renderTarget);
        ...
        void DrawFullscreenQuad(Texture2D texture, int width, int height, Effect effect, IntermediateBuffer currentBuffer)
        {
            ...
            Game1.Get.GraphicsDevice.Clear(Color.TransparentBlack); //<-- MUST do this for each target if using transparent target. 
            Game1.spriteBatch.Begin(0, BlendState.AlphaBlend, null, null, null, effect);
            Game1.spriteBatch.Draw(texture, new Rectangle(0, 0, width, height), Color.White);
