...
namespace AbstractShooter
{
    public class ASkeletalEnemyActor : AEnemyActor
    {
            ...
            for (int k = 0; k < branches; k++)
            {
                for (int i = 0; i < jointsNumber; i++)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        joints[k, i].LocalRotation += (float) Math.PI*2F*(float) k/(branches);
                    ...
                    else
                    {
                        joints[k, i] = new CSpriteComponent(this,
                            ...
                            false);
                        //joints[k, i].LocalRotation = joints[k, i].LocalRotation;
                        joints[k, i].RelativeLocation = ((i == 0) ? new Vector2(0, 0) : new Vector2(17, 0));
                    }
        ...
        {
            rootComponent.localVelocity = ObjectAngle * 35;
            ...
            for (int k = 0; k < branches; k++)
            {
                for (int i = 1; i < jointsNumber; i++)
                {
                    ...
                    {
                        joints[k, i].LocalRotation = (float)Math.Sin(sinTime + ((Math.PI / jointsNumber) * 1 * (double)i)) / 4;
                    }
                }
            }
            //Behaviour 1
            //for (int i = 0; i < jointsNumber; i++)
            //{
            //    joints[i].LocalRotation += rotationSpeed * (float)gameTime.ElapsedGameTime.TotalSeconds * StateManager.currentState.TimeScale * 0.1F;
            //}
